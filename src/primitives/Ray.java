...
public class Ray {
     ...
     * Constructor to initialize the ray
     *
     * @param normal    the normal to the vector
     * @param head      the head of the ray
     * @param direction the direction of the ray
     */
    public Ray(Vector normal, Point head, Vector direction) {
        this.direction = direction.normalize();
        double dotProduct = this.direction.dotProduct(normal);
        if (Util.isZero(dotProduct))
            this.head = head;
        else
            this.head = head.add(normal.scale(Util.alignZero(dotProduct) < 0 ? -DELTA : DELTA));
