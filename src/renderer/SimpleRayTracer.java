...
public class SimpleRayTracer extends RayTracerBase {
     ...
     *
     * @param geoPoint the intersection GeoPoint between the ray and the scene
     ...
     * @param geoPoint    the geometric point on the surface
     ...
     */
    private Double3 calcLocalEffectsSoftShadows(PointLight lightSource, GeoPoint geoPoint, Vector n, double nv, Vector v, Material material) {
        Vector l = lightSource.getL(geoPoint.point);
        ...
        for (int i = 1; i < Math.round(Math.sqrt(numOfRays)); i++) {
            for (int j = 1; j < Math.round(Math.sqrt(numOfRays)); j++) {
                ...
                ray = blackBoard.constructRay(j, i, geoPoint.point);
                if (ray != null) {
                    rays++;
                    // Add the transparency value of the intersection point to the accumulated ktr value
                    ktr = ktr.add(transparency(geoPoint, nv, ray.getDirection(), n, lightSource));
                }
            }
        }
     ...
     */
    private Double3 transparency(GeoPoint geoPoint, double nv, Vector l, Vector n, LightSource lightSource) {
        ...
        for (GeoPoint intersection : intersections) {
            if (lightSource.getDistance(geoPoint.point) > intersection.point.distance(geoPoint.point))
